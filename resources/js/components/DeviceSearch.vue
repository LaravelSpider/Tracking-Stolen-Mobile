  ...
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white shadow rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">{{ $t('search.title') }}</h3>
        <p class="text-sm text-gray-600 mb-6">{{ $t('search.description') }}</p>
        
        <form @submit.prevent="searchDevice" class="space-y-4">
          <div>
            <label for="imei" class="block text-sm font-medium text-gray-700">{{ $t('search.imei_label') }}</label>
            <input
              ...
              :placeholder="$t('search.imei_placeholder')"
              required
            />
        ...
        <div v-if="searchResult" class="mt-8">
          <div v-if="searchResult.found" class="bg-red-50 border border-red-200 rounded-md p-4">
            <div class="flex">
              ...
              <div class="ml-3">
                ...
                <div class="mt-2 text-sm text-red-700">
                  ...
                  <p><strong>{{ $t('search.status') }}:</strong> {{ $t(`status.${searchResult.device.status}`) }}</p>
                  <p><strong>{{ $t('search.reported_date') }}:</strong> {{ formatDate(searchResult.device.created_at) }}</p>
